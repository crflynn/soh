...
pub fn github_download(matches: &ArgMatches, filename: &str, url: &str) -> BoxResult<String> {
    ...
    if object_type != "list" && !overwrite && Path::new(filename).exists() {
        return Err(
            Box::try_from("File already exists; aborting. Use -o to overwrite file.").unwrap(),
        );
    }
    ...
    let response: Vec<GithubContentsEntry> =
        result.unwrap().json::<Vec<GithubContentsEntry>>().unwrap();
    ...
    for entry in response.iter() {
        ...
        if l.len() > 1 && l[0] != "" {
            files.insert(
                l[0].to_lowercase(),
                entry.download_url.as_ref().unwrap().to_string(),
            );
        }
    }
    ...
    match lower_name.as_str() {
        ...
        v => {
            ...
            let download_url = match files.get(v) {
                ...
                None => {
                    return Err(Box::try_from(
                        "Must supply a valid value. Run with `list` to see valid values",
                    )
                    .unwrap())
                }
            };
            let file_response = client
                .get(download_url)
                .header(USER_AGENT, SOH_USER_AGENT)
                .send();
